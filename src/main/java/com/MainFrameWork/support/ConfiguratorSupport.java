...
public class ConfiguratorSupport {
	...
	public String getPropertyNew(String strKey, String strFilePath) {
		try {
	...
	public void setPropertyNew(String strKey, String strValue,String strFilePath) throws Throwable {
		try {
			File f = new File(strFilePath);
			if (f.exists()) {
				FileInputStream in = new FileInputStream(f);
				...
				props.load(in);
				props.setProperty(strKey, strValue);
				props.store(new FileOutputStream(strFilePath), null);
