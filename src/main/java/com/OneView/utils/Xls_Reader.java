...
public class Xls_Reader {
	...
	public boolean setCellDatawithColRow(String sheetName,String colName,int ColRownum,int rowNum, String data){
		try{
		fis = new FileInputStream(path); 
		workbook = new XSSFWorkbook(fis);
		...
		row=sheet.getRow(ColRownum);
		for(int i=0;i<row.getLastCellNum();i++){
			//System.out.println("ColRow"+row.getCell(i).getStringCellValue().trim());
			if(row.getCell(i).getStringCellValue().trim().equals(colName))
				colNum=i;
		}
		if(colNum==-1)
